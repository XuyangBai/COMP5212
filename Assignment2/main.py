...
def train_ae(x, placeholder_x):
    ...
    start_time = time.time()
    with tf.Session() as sess:
        params, train_op, loss, feature_map, reconstructed_image = build_ae_model(placeholder_x, best_learning_rate)
        sess.run(tf.global_variables_initializer())
        sess.run(tf.local_variables_initializer())
        ae_saver = tf.train.Saver()
        for epoch in range(num_epoch):
            # this time, use all the traing set x to train the model with best hyperparameter
